...
def train(x_train, y_train, x_val, y_val, path):
    ...
    for epoch in epoch_bar:
        ...
        preds_y_val = classifier(x_val)
        acc_y_val = (preds_y_val.argmax(dim=1) == y_val).float().mean().cpu().item()

        if(acc_y_val > best_acc):
            best_acc = acc_y_val
            best_model = ({
                'epoch': epoch,
                'model': classifier,
