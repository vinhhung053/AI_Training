...
class Attention(nn.Module):
    ...
    def forward(self, x, kv_cache=None):
        ...
        # Matmul
        Q = x_.mm(self.W_q)  # Q shape [seq_sz, hidden)
        K = x_.mm(self.W_k)  # Q shape [seq_sz, hidden)
        V = x_.mm(self.W_v)   # Q shape [seq_sz, hidden)
        if kv_cache:
            # print(kv_cache)
            old_k = kv_cache[0]
            old_v = kv_cache[1]
            # print(kv_cache)
            new_k = torch.vstack([old_k, K])
            new_v = torch.vstack([old_v, V])
            K = new_k
            V = new_v
        # print(K, V)
        current_cache = [K,V]
        I = Q.mm(K.T)  # I shape [seq_sz, seq_sz]
        # Attention Mask
        if kv_cache:
...
class MultiAttention(nn.Module):
    ...
    def forward(self, x, kv_caches):
        output_attention_heads = [(head(x, kv_caches[index])) for index, head in enumerate(self.attention_heads)]
...
class GPT2Block(nn.Module):
    ...
    def forward(self, x, kv_caches=None):
        ...
        x, kv_caches_updated = self.multi_attentions(x, kv_caches)
        x = (x + residual)  # [seq_sz, len_vec]
        residual = x  # [seq_sz, len_vec]
        x = self.linear_before_gelu(x)
        x = self.gelu(x)
        x = self.linear_after_gelu(x) + residual  # Output transformer block [seq_sz, len_vec]
        return x, kv_caches_updated #[[K V] [KV] [KV]]
...
class GPT2(nn.Module):
    ...
    def positional_encoding(self, x, current_len=-1):
        seq_sz, len_vec = x.size()
    ...
    def forward(self, x, kv_caches=None):
        # print(type(kv_caches))
        current_len = -1
        # print(type(x))
        if not kv_caches:
            kv_caches = [[None] * self.num_head] * self.num_block
        else:
            current_len = x.shape[0] - 1
            x = [x[-1]]
        x = torch.tensor(x)
        # print(x.shape)
        x = self.embedding(x)
        x = x + self.positional_encoding(x, current_len)
        new_kv_caches = []
        for gpt2block, kv_cache_block in zip(self.gpt2block, kv_caches):
            x, updated_cache = gpt2block(x, kv_cache_block)
